...
public class UserResource {
	...
	@PostMapping
	public ResponseEntity<User> insert(@RequestBody User obj){//muda de json para objeto user (post no postman)
		
		obj = service.insert(obj);
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(obj.getId()).toUri();
		return ResponseEntity.created(uri).body(obj);
		//forma adequada de inserir algo no banco de dados
	}
	...
	public ResponseEntity<Void> delete(@PathVariable Long id){
		...
		service.delete(id);
		return ResponseEntity.noContent().build();//resposta sem conteúdo (postman): 204
	}
